      ...
      case ((allTxsWithValid, validIssueTxs, reissuableIssueTxs, activeLeaseTransactions, aliases), i) =>
        ...
        val tx = typeGen.getRandom match {
          ...
          case LeaseTransactionV2 =>
            ...
            recipientOpt.flatMap(recipient => logOption(LeaseTransactionV2.selfSigned(sender, 1, moreThanStandardFee, ts, recipient)))
          case LeaseCancelTransactionV2 =>
            randomFrom(activeLeaseTransactions).flatMap(lease => {
              ...
              logOption(LeaseCancelTransactionV2.selfSigned(AddressScheme.current.chainId, sender, lease.id(), moreThanStandardFee, ts))
            })
