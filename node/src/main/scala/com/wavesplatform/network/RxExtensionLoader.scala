            ...
            case LoaderState.Idle =>
              ...
              maybeKnownSigs match {
                case Some((knownSigs, optimistic)) =>
                  ...
                  ch.writeAndFlush(RawBytes(GetSignaturesSpec.messageCode, GetSignaturesSpec.serializeData(GetSignatures(knownSigs)))).addListener { f: ChannelFuture =>
                    if (!f.isSuccess) log.error(s"Error requesting signatures: $ch")
                  }
