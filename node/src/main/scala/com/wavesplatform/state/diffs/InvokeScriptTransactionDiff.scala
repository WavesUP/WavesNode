...
object InvokeScriptTransactionDiff {
  ...
  def apply(blockchain: Blockchain, height: Int)(tx: InvokeScriptTransaction): TracedResult[ValidationError, Diff] = {
    ...
    accScriptEi match {
      case Right(Some(ContractScriptImpl(version, contract, _))) =>
        ...
        contractFunc match {
          ...
          case Some(funcCall) =>
            val scriptResultE =
              stats.invokedScriptExecution.measureForType(InvokeScriptTransaction.typeId)(evalContract(version, contract))
            for {
