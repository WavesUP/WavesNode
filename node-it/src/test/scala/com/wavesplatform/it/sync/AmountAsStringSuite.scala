...
class AmountAsStringSuite extends BaseTransactionSuite {
  ...
  test("amount as string in transactions api") {
    ...
    val dataTx              = DataTransaction.selfSigned(TxVersion.V1, sender.privateKey, dataEntries, dataFee, System.currentTimeMillis()).explicitGet()
    ...
    val sponsorshipTx = SponsorFeeTransaction
      .selfSigned(TxVersion.V1, sender.privateKey, IssuedAsset(ByteStr.decodeBase58(sponsoredAssetId).get), Some(10000), sponsorFee, System.currentTimeMillis())
      .explicitGet()
