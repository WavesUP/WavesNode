...
import com.wavesplatform.api.http.ApiError.{AliasDoesNotExist, CustomValidationError}
...
//  }
//
  test("Not able to get address by an nonexistent alias") {
    val nonexestentAlias = randomAliasWithLength(15)

    assertApiError(sender.addressByAlias(nonexestentAlias),AliasDoesNotExist(Alias(nonexestentAlias)))
  }
...
//  }
  ...
  test("Not able to create alias with length < 4 and > 30") {
//    assertUnexpectedStatusCode(sender.createAlias(firstAddress, randomAliasWithLength(3), minFee), errorId = Some(199), errorMessage = Some("message"))
//    assertUnexpectedStatusCode(sender.createAlias(firstAddress, randomAliasWithLength(31), minFee), errorId = Some(199), errorMessage = Some("message"))
//    assertApiError(sender.createAlias(firstAddress, Random.alphanumeric.take(3).mkString, minFee),Some(CustomValidationError("length should be between 4 and 30")))
//    assertErrorIdAndMessage(sender.createAlias(firstAddress, Random.alphanumeric.take(3).mkString, minFee), 199,"length should be between 4 and 30")
//    assertErrorIdAndMessage(sender.createAlias(firstAddress, Random.alphanumeric.take(31).mkString, minFee), 199,"length should be between 4 and 30")
  }
  ...
  private def randomAliasWithLength(length: Int): String = {
    s"${Random.alphanumeric.take(length).mkString}".toLowerCase
  }
