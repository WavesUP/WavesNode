...
class ProtobufBytesByNetworkSuite extends FunSuite with Matchers with NodesFromDocker with WaitForHeight2 with CancelAfterFailure {
  ...
  test("able to send protobuf transaction bytes with proofs from vanilla transaction by network") {
  ...
  test("able to send protobuf transfer transaction bytes with version '2' by network") {
    val ts = System.currentTimeMillis()

    val unsigned = PBTransaction(
      chainId = AddressScheme.current.chainId,
      senderPublicKey = ByteString.copyFrom(sender.publicKey),
    ...
    val proofs = crypto.sign(sender.privateKey, unsigned.toByteArray)
    val protoTransferTx = SignedTransaction(Some(unsigned), Seq(ByteString.copyFrom(proofs)))
    sender.sendByNetwork(RawBytes(PBTransactionSpec.messageCode, protoTransferTx.toByteArray))
