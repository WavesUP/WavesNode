...
lazy val node = ProjectRef(uri("git://github.com/wavesplatform/Waves.git"), "node")

lazy val `node-it` = ProjectRef(uri("git://github.com/wavesplatform/Waves.git"), "node-it")
...
packageAll := (dex / Universal / packageZipTarball).dependsOn(root / cleanAll).value
