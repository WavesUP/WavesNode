...
class OrderHistoryBalanceSpecification
  ...
  property("Sell WAVES order - filled, buy order - partial") {
    ...
    withClue(s"counter: ${counter.order.idStr()}") {
      ...
      orderStatus(counter.order.id()) shouldBe OrderStatus.Filled(100000000)
    }
    ...
    withClue(s"submitted: ${submitted.order.idStr()}") {
      ...
      orderStatus(submitted.order.id()) shouldBe OrderStatus.PartiallyFilled(100000000)
    }
  ...
  property("Buy WAVES order - filled with 2 steps, sell order - partial") {
    ...
    orderStatus(counter.order.id()) shouldBe OrderStatus.PartiallyFilled(50000000)
    orderStatus(submitted1.order.id()) shouldBe OrderStatus.Filled(50000000)
    ...
    orderStatus(submitted1.order.id()) shouldBe OrderStatus.Filled(50000000)
    orderStatus(submitted2.order.id()) shouldBe OrderStatus.PartiallyFilled(50000000)
  ...
  property("Total execution of two counter orders and the one submitted") {
    ...
    orderStatus(submitted.order.id()) shouldBe OrderStatus.Filled(350)
  }
  ...
  property("Cancel buy order") {
    ...
    oh.processAll(OrderAdded(LimitOrder(ord1), ntpTime.getTimestamp()), OrderCanceled(LimitOrder(ord1), isSystemCancel = false, ntpTime.getTimestamp()))
