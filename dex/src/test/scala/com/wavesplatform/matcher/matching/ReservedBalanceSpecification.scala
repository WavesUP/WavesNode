...
import com.wavesplatform.matcher.AddressActor.PlaceOrder
import com.wavesplatform.matcher.AddressDirectory.Envelope
import com.wavesplatform.matcher.market.MatcherSpecLike
import com.wavesplatform.matcher.model.Events.{OrderAdded, OrderCanceled, OrderExecuted}
import com.wavesplatform.matcher.model.OrderBook._
import com.wavesplatform.matcher.model.{LevelAgg, LimitOrder, MarketOrder, OrderHistoryStub}
...
class ReservedBalanceSpecification
      ...
      new AddressDirectory(
        ...
        address =>
          Props(
            new AddressActor(
              owner = address,
              spendableBalance = _ => 0L,
              cancelTimeout = 5.seconds,
              time = ntpTime,
              orderDB = new TestOrderDB(100),
              hasOrder = _ => false,
              storeEvent = _ => Future.failed(new IllegalStateException("Should not be used in the test")),
  ...
  private implicit class DoubleOps(value: Double) {
    val waves = p.amount(value)
    val usd   = p.price(value)
    val eth   = p.amount(value)
  }
  ...
  private def placeMarketOrder(addressDir: ActorRef, order: Order): Unit = {
    addressDir ! Envelope(order.senderPublicKey, PlaceOrder(order, isMarketOrder = true))
  }
