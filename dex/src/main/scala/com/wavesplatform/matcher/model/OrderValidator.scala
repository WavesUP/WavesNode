...
object OrderValidator {
  ...
  private def validateBalance(acceptedOrder: AcceptedOrder,
        ...
        (levels.headOption, currentValue) match {
          ...
          case (Some(level), value) =>
            ...
            (isLevelPriceMatchable, remainToExecute > 0) match {
              ...
              case (false, true) => MatcherError.InvalidMarketOrderPrice(acceptedOrder.order).asLeft[Long] -> remainToExecute
    ...
    def validateTradableBalance(requiredForOrder: Map[Asset, Long]): Result[AcceptedOrder] = {
      ...
      cond(negativeBalances.isEmpty, acceptedOrder, MatcherError.BalanceNotEnough(requiredForOrder, availableBalances))
    }
  ...
  def accountStateAware(sender: Address,
    ...
    for {
      _ <- lift(acceptedOrder)
        .ensure(MatcherError.UnexpectedSender(acceptedOrder.order.sender.toAddress, sender))(_.order.sender.toAddress == sender)
        .ensure(MatcherError.ActiveOrdersLimitReached(MaxActiveOrders))(_ => activeOrderCount < MaxActiveOrders)
        .ensure(MatcherError.OrderDuplicate(acceptedOrder.order.id()))(ao => !orderExists(ao.order.id()))
