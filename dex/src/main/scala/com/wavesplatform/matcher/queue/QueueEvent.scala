...
import com.wavesplatform.matcher.model.{LimitOrder, MarketOrder}
...
object QueueEvent {
  ...
  def fromBytes(xs: Array[Byte]): QueueEvent = xs.head match {
    ...
    case 4 => val afs = Longs.fromByteArray(xs.slice(1, 8)); PlacedMarket(MarketOrder(Order.fromBytes(xs(9), xs.slice(10, Int.MaxValue)), afs))
